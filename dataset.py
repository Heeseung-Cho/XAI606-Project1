...
import os
import torch
from utils_font import pad_seq, bytes_to_file, read_split_image, round_function
from utils_font import shift_and_resize_image, normalize_image, centering_image
...
class TrainDataProvider(object):
    ...
    def compute_val_batch_num(self, batch_size):
        return int(np.ceil(len(self.val.examples) / float(batch_size)))
...
if __name__ == '__main__':
    save_fixed_sample(sample_size=64, img_size=128, data_dir='', save_dir=fixed_dir, val=True)
    save_fixed_sample(sample_size=16, img_size=128, data_dir='',
                      save_dir=fixed_dir, val=False)