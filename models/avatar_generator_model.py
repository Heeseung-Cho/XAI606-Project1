...
class Avatar_Generator_Model():
    ...
    def init_model(self, device, dropout_rate_eshared, use_wandb=True):
        ...
        #resnet = MnistResNet()
        #resnet.load_state_dict(torch.load('weights_trained/teacher.pth'))
        #resnet.fc = torch.nn.Linear(in_features=512, out_features= 2048, bias=True)
...
#            wandb.watch(resnet, log = "all")
        ...
        return (e1, e2, d1, d2, e_shared, d_shared, c_dann, discriminator1, denoiser)#, resnet)
    ...
    def load_weights(self, weights_path):
        ...
        #    torch.load(weights_path + 'resnet.pth', map_location=torch.device(self.device)))        
    ...
    def __to_cartoon(self, face, output_path):
        ...
        transform_list_faces = None #get_transforms_config_face()
    ...
    def get_loss_test_set(self, test_loader_dataset, criterion_bc, criterion_l1, criterion_l2):
        ...
        dice_coef = []
        with torch.no_grad():                
            for i, batch in enumerate(test_loader_dataset):
                
                source_batch = batch[1][:, 1:, :, :]            
                source_batch = Variable(source_batch.type(torch.Tensor))
                source_batch = source_batch.to(self.device)

                style_batch = batch[1][:,:1,:,:]
                style_batch = Variable(style_batch.type(torch.Tensor))
                style_batch = style_batch.to(self.device)

                if source_batch.shape != style_batch.shape:
                    continue

                source_enc1 = self.e1(source_batch)
                source_encoder = self.e_shared(source_enc1)
                source_decoder = self.d_shared(source_encoder)
                source_rec = self.d1(source_decoder)
                style_construct = self.d2(source_decoder)
                style_construct_enc2 = self.e2(style_construct)
                style_construct_encoder = self.e_shared(style_construct_enc2)

                style_enc2 = self.e2(style_batch)
                style_encoder = self.e_shared(style_enc2)
                style_decoder = self.d_shared(style_encoder)
                style_rec = self.d2(style_decoder)
                source_construct = self.d1(style_decoder)
                source_construct_enc1 = self.e1(source_construct)
                source_construct_encoder = self.e_shared(source_construct_enc1)

                loss_rec1 = L2_norm(source_batch, source_rec)
                loss_rec2 = L2_norm(style_batch, style_rec)
                loss_rec = loss_rec1 + loss_rec2

                loss_sem1 = L1_norm(source_encoder.detach(), style_construct_encoder)
                loss_sem2 = L1_norm(style_encoder.detach(), source_construct_encoder)
                loss_sem = loss_sem1 + loss_sem2

                #source_embedding = self.resnet(source_batch)
                #style_embedding = self.resnet(style_batch)
                #loss_teach = L1_norm(source_embedding.squeeze(), source_encoder) + L1_norm(style_embedding.squeeze(), style_encoder)
                loss_teach = torch.Tensor([1])
                loss_teach = loss_teach.to(self.device)

                output = self.discriminator1(style_construct)
                ...
                dice = compute_dice(style_batch, denorm(style_construct))
                dice_coef.append(dice.item())

        return np.mean(loss_test), np.mean(dice_coef)


    def train_crit_repeats(self, crit_opt, source_encoder, style_encoder, crit_repeats=5):

        mean_iteration_critic_loss = torch.zeros(1).to(self.device)
        for _ in range(crit_repeats):
            ### Update critic ###
            crit_opt.zero_grad()
            crit_fake_pred = self.c_dann(source_encoder) 
            crit_real_pred = self.c_dann(style_encoder)

            epsilon = torch.rand(len(style_encoder), 1,
                                device=self.device, requires_grad=True)
            gradient = get_gradient(
                self.c_dann, style_encoder, source_encoder, epsilon)
    ...
    def train_step(self, train_loader_faces, optimizers, criterion_bc, criterion_l1, criterion_l2):
        ...
        for i, batch in enumerate(train_loader_faces):
            ...
            #label_output_face = c_dann(source_encoder)
            #label_output_cartoon = c_dann(style_encoder)

            # train critic(cdann)

            loss_dann = self.train_crit_repeats(crit_opt, source_encoder, style_encoder, crit_repeats=5)
            ...
            output_real = self.discriminator1(style_batch)
            ...
            # class_faces.fill_(0)
            source_enc1 = self.e1(source_batch).detach()
            source_encoder = self.e_shared(source_enc1).detach()
            source_decoder = self.d_shared(source_encoder).detach()
            style_construct = self.d2(source_decoder).detach()
            output_fake = self.discriminator1(style_construct)
            ...
            cartoons_denoised = self.denoiser(style_rec.detach())

            # Train Denoiser

            loss_denoiser = L2_norm(style_batch, cartoons_denoised)
    ...
    def train(self):    
        ...
        model = (self.e1, self.e2, self.d1, self.d2, self.e_shared, self.d_shared, self.c_dann, self.discriminator1, self.denoiser)#self.resnet)

        train_loader_font, valid_loader_font = get_datasets(self.config.root_path, self.config.dataset_path_train, self.config.dataset_path_valid, self.config.batch_size)
        ...
        for epoch in tqdm(range(self.config.num_epochs)):
            loss_rec1, loss_rec2, loss_dann, loss_sem1, loss_sem2, loss_disc1, loss_gen1, loss_total, loss_denoiser, loss_teach, loss_disc1_real_cartoons, loss_disc1_fake_cartoons = self.train_step(train_loader_font, optimizers, criterion_bc, criterion_l1, criterion_l2)
            ...
            if self.config.save_weights and ((epoch+1) % int(self.config.num_epochs/self.config.num_backups)) == 0:
                ...
                loss_valid, dice_coef = self.get_loss_test_set(valid_loader_font, criterion_bc, criterion_l1, criterion_l2)
                dataiter = iter(train_loader_font).next()
                valid_faces = dataiter[1][:, 1:, :, :]
                valid_cartoons = dataiter[1][:, :1, :, :]
                generated_images = test_image(model, self.device, valid_faces)

                metrics_log["loss_total_valid"] = loss_valid
                metrics_log["Dice Coefficient"] = dice_coef
                metrics_log["Generated images"] = [wandb.Image(img) for img in generated_images]
                metrics_log["Original images"] = [wandb.Image(img) for img in valid_cartoons]
