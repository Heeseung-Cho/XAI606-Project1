...
def configure_model(config_file, use_wandb):
    ...
    config = dict(
        data_path=config_file["train_dataset_params"]["data_path"],
        fixed_path=config_file["train_dataset_params"]["fixed_path"],
        save_model=config_file["train_dataset_params"]["save_model"],
        save_img=config_file["train_dataset_params"]["save_img"],
        img_size=config_file["train_dataset_params"]["img_size"],
        batch_size=config_file["train_dataset_params"]["batch_size"],
        save_weights=config_file["train_dataset_params"]["save_weights"],

        fonts_num=config_file["model_hparams"]["fonts_num"],
        num_epochs=config_file["model_hparams"]["num_epochs"],
        learning_rate=config_file["model_hparams"]["learning_rate"],
    ...
    if type(resize_fix) == int:
        ...
        if verbose:
            ...
            print('resize_w:', resize_w, \
                  '[origin_w %d / origin_h %d * target_h %d]' % (origin_w, origin_h, resize_h))
